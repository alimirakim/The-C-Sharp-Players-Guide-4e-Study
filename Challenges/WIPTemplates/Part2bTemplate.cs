...
public static class PartTwoB
{
  ...
  ///  
  /// **Objectives:** 
  /// [] Create an `InventoryItem` class that represents any of the different item types. This item must represent the item's weight and volume, which it needs at a creation time (constructor).
  ...
  /// [] Build a `Pack` class that can store an array of items. The total number of items, the maximum weight, and the maximum volume are provided at creation time but cannot change afterward.
  ...
  /// 
  /// **Objectives:** 
  /// [] Override `ToString` on all of your inventory item subclasses to give them a name. For example, `new Rope().ToString()` should return "Rope".
  /// [] Override `ToString` on the `Pack` class to display the contents of the pack. If a pack contains water, rope, and two arrows, then calling `ToString` on that `Pack` object should look something like "Pack containing Water Rope Arrow Arrow".
  ...
  ///     public RobotCommand[] Commands Commands { get; } = new RobotCommand[3];
  ///     public public static void Run()
  ///     {
  ...
  /// [] Create a `RobotCommand` class with a public and abstract `public static void Run(Robot robot)` method. (The code above should compile after this step.)
  /// [] Make `OnCommand` and `OffCommand` classes that inherit from `RobotCommand1 and turn the robot on or off.
